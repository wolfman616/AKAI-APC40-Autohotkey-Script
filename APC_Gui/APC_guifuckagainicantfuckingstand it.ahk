...
global Rot_Num_in, global Rot_Led_in, global Rot_Num, global Rot_Led, global RotLed_Old , global xnow, global ynow, global Y_pos_1, global X_pos_1, global X_pos_2, global sByte, global chan_num := "1", global string, global MiDi_inout, global chan_num
	...
	
	{
		...
		{	
				...
				ROT_%Rot_N% 		:= 	Rot_Led
					...
					;msgbox % sByte " " rot_num
				if Rot_N between 17 and 25 
				{
								byte1 	:= Rot_Num + 7
		;msgbox % "GAYKNObS " Rot_N
 					sByte := chan_num + 175 
byte2 		:= FLOOR(9.07 * Rot_Led)	; value

				}
				else {
								byte1 		:= Rot_Num	; knob num
		sByte 		:= 176	; knob num
		byte2 		:= round(format("{:g}", Rot_Led)) ; value

				}
				string 		:= (sByte . "," . byte1 . "," . byte2)
				Send_WM_COPYDATA(string, MiDi_inout)			;send midi						
				tooltip % Rot_Num "`n" string "`n" rot_n
								gosub Knob_Upd8 ;update gui lights
oldrot%Rot_N% := Rot_%Rot_N%
		}	
		;TOOLTIP % Rot_N "`n" Rot_Num
	}
}	
return
