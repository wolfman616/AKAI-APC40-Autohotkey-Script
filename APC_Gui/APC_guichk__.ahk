...
OnMessage(0x4a, "Receive_WM_COPYDATA") ; 0x4a is WM_COPYDATA ;; window message for the mouse left click
...
if (chan_num < 9) 										  {
	...
	Loop, 	8											 {
		...
		Send_WM_COPYDATA((sByte . "," . byte1 . "," . byte2), MiDi_inout)	
		gosub, Knob_Upd8
}	}
...
; gosub, 	Knob_Upd8
; return
init_RotLoopIteration:
RotLEDsAll  :=	("cWhole" .   A_index)
...
gui, 	%RotLEDsAll%:	New, +OwnDialogs -Caption +ParentAPCBackMain -DPIScale +disabled -SysMenu +ToolWindow, midi ; classname blacklisted from Aeroglass, will be using for all controls 
gui, 	%RotLEDsAll%:	color, 000000	;	DllCall("SetParent", "uint", %RotLEDsAll%, "uint", MainhWnd)
gui, 	%RotLEDsAll%:	Add, Picture,X0 Y0 w80 h80 BackgroundTrans , %knob1%	; 
gui, 	%RotLEDsAll%:	Show, noactivate %X_pos_2% %Y_pos_1% w80 h80 , midi
gui, 	%RotLEDsAll%: 	+disabled +LastFound -Caption +hwnd%RotLEDsAll%, midi ; 
WinSet, Transcolor, 	000000 		; 	doesnt need full alpha so this gui is done for now
%RotLEDsAll%dc 	:= 		DllCall("GetDC", "UInt", %RotLEDsAll%)
gui, 	APCBackMain:	Show, noactivate x433 y433 w1110 h666, APCBackMain
gui, 	APCBackMain: 	+LastFound -Caption
gui, 	%gaP_Wince%:	New, +ToolWindow +OwnDialogs -Caption -DPIScale +ParentAPCBackMain +AlwaysOnTop -SysMenu +E0x80000, midi ;+hwndhwnd%gaP_Wince% 
gui, 	%gaP_Wince%: 	+disabled +LastFound -Caption

hwnd%gaP_Wince% := 	WinExist()
pToken%A_index% := 	Gdip_Startup()
pImage%A_index% := 	Gdip_LoadImageFromFile("C:\Script\AHK\Z_MIDI_IN_OUT\APC_Gui\Images\mask.png")
...
gui, 	%gaP_Wince%:	+LastFound -Caption +disabled 	;	WinSet, Transcolor, ffffff
gui, 	%RotLEDsAll%:	Hide
pBitmap%A_index%:= 	Gdip_CreateBitmapFromFile("C:\Script\AHK\Z_MIDI_IN_OUT\APC_Gui\Images\mask.png")	 ;	Gdip_GetRotatedDimensions(80, 80, 90, rw1, rh1)
...
DllCall("gdi32.dll\StretchBlt", "Uint", mDC%A_index%, "Int", 0, "Int", 0, "Int", nH , "Int", nH , "UInt", %RotLEDsAll%dc, "UInt", 0, "UInt", 0, "Int", nW, "Int", nH, "UInt", "0x00440328")
...
; {
	; tooltip, No changes no save,,,2
	; sleep 1700
; } else if donotsavenow {
	; RotLedsSaved := shitarsecunt
	; filedelete, C:\Script\AHK\Z_MIDI_IN_OUT\APC_Gui\knobs.ini
	; tooltip, deleting CANCERs,,,2	
; }
; sleep 1000
; tooltip,,,,2
; return
