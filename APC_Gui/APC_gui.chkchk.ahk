...
global yy
global total 
global Degree_Increment 
global Rotary_string2save 
global RotLedsSaved
global shitarsecunt
global ALL_O 
global mW := 60
...
Loop, 16 { ; rotarys :
	...
	gui, 	%C_Whole%:New, +OwnDialogs -Caption +ParentAPCBackMain -DPIScale +disabled -SysMenu +ToolWindow, midi ; classname is blacklisted from Aeroglass, will be using for all controls ;Layerr%A_index% 
...
Knob_Upd8:
 if !Rot_%Rot_N%
 Rot_%Rot_N% := 7
;msgbox % Rot_P ;if chan_num;nH:=80, nw:=80;pBitmap%Rot_P% := Gdip_CreateBitmapFromFile(Rot_Mask);Gdip_GetRotatedDimensions(80, 80, 90, rw1, rh1);Gdip_GetRotatedDimensions(0, 0, 57.29578 * atan(0/0), rw, rh);G%Rot_P%:=Gdip_GraphicsFromHDC(mdc%Rot_P%)
...
Gdip_TranslateWorldTransform(G%Rot_P%, -40, -40) ;win_move((hwndgapWince%Rot_P%) ,"","","80","80")
...
global yyy := lParam >> 16
   ...
   ; ToolTip You left-clicked in Gui window #%A_Gui% at client coordinates %X%x%Y%.%Ctrl%,3800,1800,3
; if	!counta
	 ; counta := 1
; else counta := counta +1
;mousegetpos, xxx, yyy ;wnd, cnt
...
if ((yyy < 94) && (xxx < 685)) {
	...
	tooltip, % xxx " " yyy,,,2
		...
		if xxx between 758 and 842
			...
			Rot_P	:=	15
		else if xxx between 1011 and 1093 ;1792 mmsg offset 
			Rot_P	:=	16
		else {
