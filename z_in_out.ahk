...
ini_Read() ; load previous midi settings
...
menu, tray, add, % "Master Volume Tip", TTMVolTogl
menu, tray, Standard
...
gosub, MiDi_IN_GO ; opens the midi input port listening routine
gosub, MiDi_OUT_GO ; opens the midi out port
;SendLevel 99
;Send, #^r
vars:
global h_midiout
...
if(byte10ld=23) && (statusbyte=176) { ;CHAN 1 SELECT
 ch :=  1 ;(statusbyte - 175)
	...
	{
		messagedata :=  ("CH" . ch ",chn")
		Send_WM_COPYDATA(messagedata, GuiScriptTitle)
		;traytip CHANNEL CHANGED you spastic fucking chn, CHANNEL %CHANNEL%
...
ini_Read() ; also set up the tray Menu
{
...
MiDi_OUT_GO: ; Function to load new settings from midi out menu item
OpenCloseMidiAPI()
